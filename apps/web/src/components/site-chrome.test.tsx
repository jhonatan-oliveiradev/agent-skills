...
describe("editorial site chrome", () => {
  ...
  it.each([
    ...
    expect(within(dialog).getByText("38 SKILLS")).toBeInTheDocument();
    expect(within(dialog).getByText("8 PACKS")).toBeInTheDocument();
  ...
  it.each([
    ...
    expect(within(collection!).getByText(/38 skills/i)).toBeInTheDocument();
    expect(within(collection!).getByText(/8 packs|8 pacotes/i)).toBeInTheDocument();
  });
});
