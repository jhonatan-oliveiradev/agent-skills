...
describe("sitewide UI hardening", () => {
  ...
  it("reflows narrow pack dossiers with selectors strong enough to win the existing cascade", async () => {
    ...
    expect(source).toMatch(/@container\s*\(max-width:\s*46rem\)[\s\S]*\.home-pack-archive \.home-pack-dossier__body\s*\{[\s\S]*grid-template-columns:\s*1fr/);
    expect(source).toMatch(/@container\s*\(max-width:\s*46rem\)[\s\S]*\.home-pack-archive \.home-pack-dossier h3[\s\S]*max-width:\s*100%/);
