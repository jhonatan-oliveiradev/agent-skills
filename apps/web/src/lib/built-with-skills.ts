...
const cases = [
  ...
  },
  {
    ...
    "date": "2026-09-03",
    "sourcePath": strataArchitectureCaseSourcePath,
    "evidenceClass": "real-use",
    ...
    "locales": {
      "en": {
        ...
        "decisions": [
          {
            "title": "Keep the least-distributed architecture that satisfies the forces",
            "summary": "Retain the existing modular Next.js + Prisma monolith because tenant isolation, lifecycle consistency, and transactional invariants did not require an independently deployed service."
          },
          {
            "title": "Make the feature own its read contract",
            "summary": "Move the narrow read source types into the Contracts feature so the Prisma repository satisfies that contract instead of exporting persistence-owned payload types upward."
          },
          {
            "title": "Put replacement legality in the domain",
            "summary": "Define replacement eligibility once in the canonical status policy and let presentation delegate to it instead of duplicating FINALIZED/SENT lifecycle semantics."
          },
          {
            "title": "Refactor only the proven seam",
            "summary": "Use two RED architecture contracts and a six-file follow-up, while explicitly deferring a generic retry abstraction until a second real operation needs the same semantics."
          }
        ],
        "results": [
      ...
      "pt-BR": {
        ...
        "decisions": [
          {
            "title": "Manter a arquitetura menos distribuída que atende às forças reais",
            "summary": "Preservar o monólito modular existente em Next.js + Prisma porque isolamento por tenant, consistência de lifecycle e invariantes transacionais não exigiam um serviço com deploy independente."
          },
          {
            "title": "Fazer a feature ser dona do contrato de leitura",
            "summary": "Mover os tipos estreitos de source do read model para Contracts para que o repository Prisma satisfaça esse contrato, em vez de exportar payloads de persistência para cima."
          },
          {
            "title": "Colocar a legalidade de replacement no domínio",
            "summary": "Definir a elegibilidade de replacement uma única vez na status policy canônica e fazer presentation delegar a ela, sem duplicar a semântica FINALIZED/SENT."
          },
          {
            "title": "Refatorar apenas a seam comprovada",
            "summary": "Usar dois contratos RED de arquitetura e uma follow-up de seis arquivos, adiando explicitamente uma abstração genérica de retry até existir uma segunda operação real com a mesma semântica."
          }
        ],
        "results": [
  ...
  },
  {
    ...
    locales: {
      en: {
        ...
        summary:
          "A real Tsukihara maintenance slice used the complete Motion pack to tighten GSAP lifecycle ownership, reduce WebGL work under reduced motion, preserve cinematic continuity, and reconstruct the eclipse moon from reference-supported Three.js geometry.",
        challenge:
          "Improve an existing GSAP, Lenis, and React Three Fiber experience without redesigning it: honor reduced-motion intent in the persistent world, remove unnecessary ScrollTrigger work, keep one cinematic ownership model, and make the procedural eclipse moon evidence-limited rather than speculative.",
        decisions: [
          {
            title: "Make reduced motion a runtime contract",
            summary:
              "Observe prefers-reduced-motion in the persistent R3F world, switch that mode to demand rendering and lower DPR, and remove continuous petals and pointer embers instead of merely disabling the hero timeline.",
          },
          {
            title: "Keep GSAP lifecycle aligned with actual ownership",
            summary:
              "Do not schedule ScrollTrigger.refresh() when the reduced-motion hero path has no active timeline to refresh.",
          },
          {
            title: "Preserve one cinematic system",
            summary:
              "Keep hero, eclipse state, camera movement, and persistent WebGL world coordinated rather than adding another independent decorative animation layer.",
          },
          {
            title: "Reconstruct only what the reference proves",
            summary:
              "Model the visible eclipse as a deterministic frontal disc/shadow plus crimson RingGeometry rim, explicitly avoiding unseen backside, crater topology, or 360-degree fidelity claims.",
          },
        ],
        results: [
      ...
      "pt-BR": {
        ...
        summary:
          "Um slice real de manutenção no Tsukihara usou o pack completo de Motion para reforçar o lifecycle do GSAP, reduzir trabalho WebGL em reduced motion, preservar continuidade cinematográfica e reconstruir a lua do eclipse com geometria Three.js sustentada pela referência.",
        challenge:
          "Melhorar uma experiência existente com GSAP, Lenis e React Three Fiber sem redesenhá-la: respeitar reduced motion no world persistente, remover trabalho desnecessário de ScrollTrigger, manter um único modelo de ownership cinematográfico e limitar a lua procedural ao que a referência realmente comprova.",
        decisions: [
          {
            title: "Transformar reduced motion em contrato de runtime",
            summary:
              "Observar prefers-reduced-motion no world R3F persistente, usar renderização sob demanda e DPR menor nesse modo e remover pétalas contínuas e pointer embers em vez de apenas desligar a timeline da hero.",
          },
          {
            title: "Alinhar o lifecycle do GSAP ao ownership real",
            summary:
              "Não agendar ScrollTrigger.refresh() quando o caminho reduced-motion da hero não possui timeline ativa para atualizar.",
          },
          {
            title: "Preservar um único sistema cinematográfico",
            summary:
              "Manter hero, estado de eclipse, movimento de câmera e world WebGL persistente coordenados, sem adicionar uma camada decorativa independente.",
          },
          {
            title: "Reconstruir somente o que a referência comprova",
            summary:
              "Modelar o eclipse visível como disco/sombra frontal determinístico com rim carmesim em RingGeometry, evitando alegações sobre backside, topologia de crateras ou fidelidade 360 graus não observáveis.",
          },
        ],
        results: [
