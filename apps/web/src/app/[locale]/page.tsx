...
export default async function HomePage({ params }: HomePageProps) {
  ...
  return (
    <>
        ...
        <div className="shell">
          ...
          <div className="home-proof-v2__case">
            <div className="home-proof-v2__facts">
              <div>
                <span>{evidence.proof.challengeLabel}</span>
                <p>{evidence.proof.challenge}</p>
              </div>
              <div>
              ...
              <div>
                <span>{evidence.proof.outcomeLabel}</span>
                <p>{evidence.proof.outcome}</p>
              </div>
              <div>
              ...
              <figure className="home-proof-preview home-proof-preview--after">
                ...
                <div className="home-proof-preview__browser">
                  ...
                  <div className="home-proof-preview__after-layout">
                    ...
                    <div className="home-proof-preview__manifesto">
                      <b />
                      <strong />
                      <strong />
                      <span />
      ...
      <section className="home-transformation" data-home-section="transformation">
        <div className="shell">
          <div className="home-editorial-heading">
            <div>
              <p className="eyebrow">{evidence.transformation.eyebrow}</p>
              <h2>{evidence.transformation.title}</h2>
            </div>
      ...
      <section className="home-methods-v2" data-home-section="methods">
        <div className="shell">
          <div className="home-editorial-heading">
            <div>
              <p className="eyebrow">{evidence.methods.eyebrow}</p>
              <h2>{evidence.methods.title}</h2>
            </div>
            <div>
              <p>{evidence.methods.summary}</p>
              <Link href={localizePath("/skills", locale) as Route}>{evidence.methods.viewAll} →</Link>
            </div>
          </div>
              ...
              <li key={method.slug}>
                <Link href={`/${locale}/skills/${method.slug}` as Route}>
                  ...
                  <span className="home-method-index__name">
                    <strong>{method.skill.displayName}</strong>
      ...
      <section className="home-packs-v2" data-home-section="packs">
        <div className="shell">
          <div className="home-editorial-heading">
            <div>
              <p className="eyebrow">{evidence.packs.eyebrow}</p>
              <h2>{evidence.packs.title}</h2>
            </div>
            <div>
              <p>{evidence.packs.summary}</p>
              <Link href={localizePath("/packs", locale) as Route}>{evidence.packs.viewAll} →</Link>
            </div>
          </div>
          ...
          <div className="home-pack-rail">
            {activePacks.map((pack) => (
              <article key={pack.slug}>
                ...
                <Link href={`/${locale}/packs/${pack.slug}` as Route}>
                  {evidence.packs.view} →
                </Link>
              </article>
            ))}
      ...
      <section className="home-ledger" data-home-section="ledger">
        <div className="shell">
          <div className="home-editorial-heading">
            <div>
              <p className="eyebrow">{evidence.ledger.eyebrow}</p>
              <h2>{evidence.ledger.title}</h2>
            </div>
            <div>
              <p>{evidence.ledger.summary}</p>
              <Link href={localizePath("/built-with-skills", locale) as Route}>{evidence.ledger.viewAll} →</Link>
            </div>
          </div>
          ...
          <div className="home-evidence-ledger">
            <table>
              <thead>
                <tr>
                  <th scope="col">{evidence.ledger.methodLabel}</th>
                  <th scope="col">{evidence.ledger.usedInLabel}</th>
                  <th scope="col">{evidence.ledger.evidenceLabel}</th>
                </tr>
              </thead>
              <tbody>
                {ledgerRows.map((row) => (
                  <tr key={row.method.slug}>
                    <td>
                      <Link href={`/${locale}/skills/${row.method.slug}` as Route}>
                        {row.method.skill.displayName}
                      </Link>
                    </td>
                    <td>
                      {row.usedInHref ? <Link href={row.usedInHref}>{row.usedIn}</Link> : row.usedIn}
                    </td>
                    <td>
                      {row.external ? (
